...
- ダウンロードページに `Code signing policy`、公開ソース、署名担当ロール、プライバシーポリシーを掲載。
- SignPath Foundation承認前であることを明記し、未署名配布物を署名済みと誤認させない表現に統一。
- 公開版v3.2.7の安全なセキュリティ警告案内を維持し、セキュリティ機能を無効化する手順は掲載しない。

### バックアップ

- `backups/v3.2.6-signpath-policy-pre_20260731/`
...
## [v3.2.7] - 2026-07-31 - SAKURA-Remote公開ページ安全修正

### 変更内容

- 緊急ロールバック時に本番`remote.html`だけが旧版へ戻り、Git管理版と不整合になっていた問題を修正。
- 公開ページから第三者製品名を撤去し、表示名を`SAKURA-Remote` / `さくらリモート`へ統一。
- Smart App ControlなどWindowsのセキュリティ機能を無効化する案内を撤去。
- ブロック時はセキュリティ機能をオフにせず、インストールを中止してサポートへ連絡する安全案内へ統一。
- Windowsインストーラーは署名・ブランド・実機テストが完了するまで従来版のまま凍結し、新版をアップロードしない。
- 変更前バックアップ: `backups/v3.2.7-remote-safety-hotfix-pre_20260731/`
- Claude Codeレビュー: 計画は`CONDITIONAL GO`、インストーラーの本番アップロードは`NO-GO`。

### 本番アップロード対象

- `remote.html` → `https://sakuranet-co.jp/remote.html`

## [v3.2.6] - 2026-07-31 - Windows版緊急ロールバック

### 変更内容

- ブランド要件違反のため、公式クライアント配布を直ちに停止。
- Windows版リンクを従来の`SAKURA-Remote-Setup.exe`へ復元。
- 公開ページ上の第三者製品名を全撤去。
- Windowsのセキュリティ機能を無効化・回避する案内は復活させず、安全な停止案内へ変更。
- 緊急ロールバック前バックアップ: `backups/v3.2.6-emergency-rollback-pre_20260731/`
...
**contact.html**
- 「ご注意」欄の先頭に、お電話でのお問い合わせに関する注意文を追加。
- 電話問い合わせはAI受付で内容を預かり、内容確認後に担当者より折り返す旨を明記。
- 無言、メッセージ未確認、さくらインターネット株式会社様への間違い電話、当社サービスと関係のない営業・勧誘等は折り返しできない場合がある旨を明記。
- アポイントのない訪問・来店は、セキュリティ管理上、スタッフ在席時でも受付・面談・対応できない旨を追記。
- 既存の「さくらインターネット株式会社様へのお問い合わせについて」「セキュリティチェック・通報について」は維持。

### バックアップ

- `backups/v_contact_ai_notice_080709_102751/contact.html`
- `backups/v_contact_ai_notice_080709_102751/RELEASE_NOTES.md`
- `backups/v_contact_visit_notice_080709_110246/contact.html`
- `backups/v_contact_visit_notice_080709_110246/RELEASE_NOTES.md`
...
**index.html**
- トップページ Services 内の SAKURA BLOOM カードアイコンを、絵文字ベースの簡易表示から専用SVGビジュアルへ変更。
- LINE吹き出し・予約カレンダー・管理画面カードを組み合わせ、LINE公式活用システムであることが一目で伝わる表現に改善。
- 既存カードレイアウト・リンク・本文・SAKURA BLOOM紹介ページへの導線は変更なし。

### バックアップ
- ackups/v3.2.1_pre-bloom-icon/
...
**contact.html**
- 「ご注意」セクション内の既存通知（さくらインターネット株式会社様への問い合わせ案内）の直下に、新たに「セキュリティチェック・通報について」カードを追加。
- 内容: 当社では問い合わせ内容を独自のセキュリティシステム及び迷惑メール判定（AI）で精査しており、不適切・なりすまし・営業勧誘・迷惑メールと判定された場合は返答せず専門機関へ自動通報する旨を明示。通報により送信元メールアドレス・ドメインが専門機関のブラックリストへ登録される可能性も告知。
- 既存の「さくらインターネット様」案内カードと同じビジュアルスタイル（左辺アクセントカラー、Before Contact 表記）を踏襲し、ラベルは「Security Check」。

### バックアップ

- `backups/v3.1.6_pre-contact-security-notice/contact.html`
...
- ローカル静的レンダリング（Python http.server + Chrome dev-browser PC 1440×900）で「ご注意」配下に 2 つのカード（Before Contact / Security Check）が縦に並ぶレイアウトを確認済み。
- 既存の「さくらインターネット様」案内は無傷で維持。
...
## [v3.0.24] - 2026-04-27 - 法人ITサポート詳細ページを再構築
### 変更内容
- `posts/service7.html` を旧記事形式から、法人ITサポート専用ページへ全面刷新
- PC、ネットワーク、サーバー、クラウド、セキュリティ、遠隔サポート、訪問対応、月額保守の対応範囲を整理
- 旧ページの空リンク化していた料金表・約款リンクを削除し、契約前に個別資料で確認する注意書きへ変更
- 強く出すぎていた解約注意文をページ本文から外し、契約時資料・担当窓口案内を優先する表現に整理
- `Service` / `BreadcrumbList` / `FAQPage` の構造化データを追加
- フッターを `SUPPORT` / `SYSTEM` / `© 2026 SAKURA-NET Inc. All Rights Reserved.` 表記に統一

### バックアップ
- `backups/v3.0.24_pre-it-support-page/` に更新前バックアップを保存

**変更ファイル:** posts/service7.html / RELEASE_NOTES.md
...
## [v3.1.8] - 2026-06-01 - 予約フォームに参加人数を追加

### 変更内容

**reservation.php**
- 予約フォームに `参加人数` の必須セレクトを追加。
- 選択肢は `1名` / `2名` / `3名` / `4名` / `5名` / `6名以上`。

**reservation_send.php**
- `participant_count` を必須項目として受け取り、1〜6 の範囲でバリデーション。
- GAS 送信payloadへ `participantCount` を追加。
- 予約本文に `参加人数: n名` または `参加人数: 6名以上` を記録。

### バックアップ

- `backups/v3.1.8_reservation_participants_20260601/reservation.php`
- `backups/v3.1.8_reservation_participants_20260601/reservation_send.php`
- `backups/v3.1.8_reservation_participants_20260601/RELEASE_NOTES.md`
...
## [v3.3.1] - 2026-08-21 - お問い合わせフォームBot対策の共有サーバー向け安全処理

### 変更内容
- `contact_send.php` のレート制限保存ディレクトリ作成処理で、共有サーバー環境でもPHP警告が画面に出ないように安全処理を追加。
...
## [v3.3.2] - 2026-09-07 - SHOPご利用対象の注意書き追加

### 変更内容
- `shop/index.php` のヒーロー直下に、さくらねっとサービスをご利用中、または当社より案内を受けた法人のお客様向けのオンライン決済ページである旨を追記。
- 初めてのお客様向けに本体サイトのお問い合わせフォームへの導線を追加。

### バックアップ
- `backups/v3.3.2_shop_customer_notice_0907154244/`
